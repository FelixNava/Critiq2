...
> **STILL OPEN in the epic → next Phase-38 work (new branch off review-for-main):** **38c** audio player (`GET /api/recording/[id]/audio`, server-proxied private bytes + Range) + **38d** synced transcript (audio-time, click-to-seek). Both gated on a real **≥10-min recording** (iPhone for the MP4 codec path) + the playback/seek **device gate**. The audio-contract design half is locked in `docs/CALL_ANALYSIS_AUDIO_CONTRACT.md` (Blob private→server-proxy; audio-time timeline; word re-basing formula); the empirical half (cross-segment + iOS decode) needs Felix's recording.
