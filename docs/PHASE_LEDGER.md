...
> **38c DONE (single-segment) — built + preview-verified; PR [#39](https://github.com/FelixNava/Critiq2/pull/39) → review-for-main (NOT yet merged: awaiting Felix's iOS device gate + merge).** `GET /api/recording/[id]/audio` server-proxies private chunk bytes with full HTTP Range (200/206/`Content-Range`/`Content-Length`, the iOS `bytes=0-1` probe, 416); a pure gap-aware `audioPlan` (dense single segment → `ready`; multi-segment / non-dense / empty → honest **422** + an explicit page message, never a corrupt concat); native `<audio controls>` player (the synced scrubber/markers/keyboard/speed = Phase 40); Content-Type inferred from the chunk pathname; real captured duration shown as a label (MediaRecorder-WebM has no in-header duration); reuses the Phase 38f authenticated private-read. **Verified on the preview against real recordings:** 401 unauth, 200 + valid WebM EBML, 206 iOS-probe + mid-range, two adjacent ranges concatenate byte-identical to the full body, 416 over-EOF, 422 honest-degrade, the `<audio>` element decodes (`readyState 4`). `scripts/verify-phase38c.ts` 41/41; build clean. **No schema / CSP / env change.** DEC-060.
> **STILL OPEN in the epic → next Phase-38 work:** **38d** synced transcript (audio-time, click-to-seek, current-line highlight) + the **device gate Felix owns**: iOS Safari decode of MP4-origin chunks + the **multi-segment** assembly boundary + lock/background playback (`docs/CALL_ANALYSIS_AUDIO_CONTRACT.md` §D1–D4) — both need a real **≥10-min iPhone recording** (none of the 21 existing recordings is multi-segment). The audio-contract design half (§A–C) is locked + now shipped in 38c; the empirical half (§D) needs Felix's recording.
