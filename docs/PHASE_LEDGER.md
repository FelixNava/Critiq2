...
- Last updated: 2026-06-20 — **Phase 37 (Account & data controls + workspace nav) ✅ DONE** — merged to review-for-main (PR #37, squash `6ab6d9b`), verified live (P0 hard-delete end-to-end). `/profile` = account basics + read-only learned model (rep_summaries) + intake status; header workspace nav (Dashboard·Accounts·Recordings); the P0 hard-delete (type-email confirm → Vercel Blob + cascade purge, admin-blocked) + honest privacy copy. review-for-main holds Phases 2–21 + 23–27 + 33 + 34 + 35 + 37; master holds 2–13. **NEXT (pending Felix's go): the CALL-ANALYSIS experience** — robust approach delivered via the `robust-call-analysis-approach` workflow: `/recordings/[id]` detail (playback + synced transcript + full assessment + evidence linking + refine loop), a shared call-pill **Analysis button**, fix the assign-flow bug, and close the score↔memory loop (scoring ignores memory in / score never feeds memory out today). 7 phases, 5 full-auto-able; Felix owns the audio device-gate + AI-feedback calibration (6–7). Model rec = Opus 4.8 for scoring+coaching. See memory `project_critiq_status`.
- Last updated: 2026-06-19 (scheduled task `critiq-fullauto-34-35`, firing #4 — **Phase 35c** merged to review-for-main, PR #36, squash `8cef470`; **Phase 34 + Phase 35 are now BOTH FULLY DONE** → the task self-disables). review-for-main now holds Phases 2–21 + 23–27 + 33 + 34 + 35; master holds 2–13. Phase 35c wired the Phase 25 working-memory builder + the Phase 35a rep/account free-text context into all four AI consumers (pre-call/script/debrief/coaching), Phase 26 guard intact. Prior context below is retained but superseded by this line.
- Last updated (prior): 2026-06-18 (aggressive-auto — **Phase 26 Hallucination Guard** merged to review-for-main, PR #31; review-for-main held Phases 2–21 + 23 + 24 + 25 + 26 + 33; master holds 2–13). The conservative pre-output validator (strips unsourced personal references; consumes the Phase 23/24 source-tagged facts/traits + Phase 25 raw interactions as the grounded corpus) is wired fail-safe into the coaching output path. **🎯 AGGRESSIVE-AUTO SCOPE COMPLETE** — the scheduled task `critiq-fullauto-aggressive-15-26` built its full scope (value engine 15–17, interaction loop 18–21, memory moat 23–25, the landing redesign, and now the hallucination guard 26); the task self-disables. Phase 22 (email drafting) remains deferred post-launch. NEXT (whenever Felix resumes): Phase 27 (cold-start UX) onward, + the FLAGGED AI-quality validation pass across 15–26 with the expert coach, + the pre-beta prod-DB/Blob separation.
...
## Phase 35 — Add Context + Import Past Transcripts/Notes + Wire Working Memory into Coaching ✅ DONE (35a/b + 35c)

> **STATUS (2026-06-19, scheduled task `critiq-fullauto-34-35`, firing #4):** **FULLY DONE.** 35a/b merged firing #3 (PR [#35](https://github.com/FelixNava/Critiq2/pull/35), squash `22a6369`); **35c (wire Phase 25 working memory + the 35a context into the four AI consumers) DONE firing #4** — merged `review-for-main` (PR [#36](https://github.com/FelixNava/Critiq2/pull/36), squash `8cef470`). **🎯 This completes BOTH Phase 34 and Phase 35 → the `critiq-fullauto-34-35` task self-disables.**
>
> **35c shipped (NO schema change, NO UI change):** a new consumer adapter `buildConsumerWorkingMemory` (`src/lib/workingmemory/forConsumer.ts`) reuses the Phase-25 readers + pure assembler to give each consumer (a) the RICHER rep profile (learned P24 → intake P18 → none, identical to the old block at cold start) for the *cached* layer 2, and (b) a VOLATILE memory context block (rep + account free-text context + the account semantic summary + last-3 raw interactions, budgeted; account summary falls back to `account_records.summary`). A pure `renderAccountKnowledge` (`src/lib/workingmemory/consumerPrompt.ts`) renders the account-knowledge body — memory → bare summary → cold-start note (byte-identical fallback). All four consumers (pre-call 18 / script 19 / debrief 20 / coaching 21) wire it uniformly; cache split intact (rep profile cached, memory volatile). The caller owns the access boundary (each `generate()` already authorized the account → passed in, no re-fetch). **Phase 26 guard untouched** — every reference the memory context can introduce is already in `buildGroundedSources`, so it's never falsely redacted (probe: a grounded name survives, 0 dropped). Verify: typecheck+build clean; `verify-phase35c` 32/32; `verify-phase18/19/20/21/25/26` no regression; throwaway real-PG+real-Claude probe (assembly from real rows, access boundary, grounding inclusion, guard keeps grounded refs, real before/after showing memory sharpening coaching — the "after" cites the account's spring reorder cycle + the multi-touch history). High-effort `/code-review` (7 angles) → no correctness bugs; efficiency/cleanup fixes applied in-branch. **DEC-053/054.** **FLAGGED for Felix + the expert coach (NOT gated):** the relaxed gate proves it runs, not that coaching improved (validate on real inputs); rep context is account-AGNOSTIC (a typed specific grounds in every account's coaching — DEC-054); debrief sees prior interactions as background (watch for a prior-call detail in a current-call recap); 35c has no UI so no `/design-critique`.
>
> **35a/b shipped:** `rep_context` table (rep-private) + shared `account_records.context` column (mig 0018); `src/lib/context.ts` (access-gated get/set); routes `GET`/`PUT /api/profile/context` + `GET`/`PUT /api/accounts/[id]/context`; UI `/profile` + the account "What you've told Critiq" card via a shared `ContextEditor` + a `Profile` nav link; rep + account context wired into the Phase 26 grounded corpus with synthetic source tags (`rep-context`/`account-context`) so context is grounded, never redacted. Import: `src/lib/debrief/import.ts` + `POST /api/accounts/[id]/import` (structures the paste via the SAME Phase 20 generator → completed debrief → fires 23/24 consolidation via `after()`; no new pipeline) + `/accounts/[id]/import` page + `ImportPanel`. Verify (relaxed): typecheck+build clean; verify-phase35 23/23; verify-phase20/21/25/26 no-regression; throwaway real-PG probe (context round-trip, access boundary, grounding inclusion, import→structured-debrief→consolidation +1). `/code-review` high-effort + `/design-critique` (static, P0/P1 addressed in-branch). **FLAGGED:** `UI_VERIFY_PENDING=35ab` (live auth UI eyeball owed); rep-context grounding is account-agnostic (a typed specific grounds in every account's coaching); import structuring quality on raw transcripts; AI value isn't proven until 35c wires working memory in.
>
> **35c REMAINING (next firing):** plug `buildWorkingMemory` (Phase 25) into pre-call (18) / script (19) / debrief (20) / coaching (21) via `buildCachedSystem`, keeping the Phase 26 guard in the path, so the assembled set — INCLUDING the new rep + account context (a working-memory source must be added for context) — drives the prompts. Touches all four consumers — SURGICAL; run verify-phase18/19/20/21/25/26 for no regression; do not regress the un-recorded/cold-start loop.
...
## Build sequence — LOCKED 2026-06-23 (two design invariants + the order)

Felix locked two invariants that drive the remaining call-analysis + memory build:
1. **Everything feeds everything.** Every input (recording, score, transcript, debrief, context) flows into the shared account + rep + working memory; every output (brief, script, coaching, the account card) reads all of it. *(Today ~half-true: debriefs/context/imports feed memory; recordings/scores/transcripts are the dead-end — Phases 42 + 44 close it. Symptom Felix hit 2026-06-23: a scored recording shows on its own page but the account page still says "no calls recorded" / "0 of 10 calls" — `getAccountForUser` returns `loggedCalls: 0` hardcoded; "Last interaction"/"How they buy" are static ColdStart placeholders.)*
2. **Capture is effortless.** Anywhere there's a text/recap input (debrief, account/rep context, import, pre-call), there's also a mic; the AI transcribes AND routes the content to the right place(s). Phase 45 = the easy voice primitive; Phase 46 = the AI router.

**Build order (re-sequenced from the numeric 39→44):**
- **Block A (start here):** **42** (account becomes call-aware; kills "no calls recorded"; wires `loggedCalls`) · **45** (voice-input primitive). Both auto-buildable + directly visible.
- **Block B (legible + trustworthy score):** **39** (coachable moments) · **41** (metrics + methodology + next-steps) · the **scoring AI-quality pass** (expert coach) — gates Block C.
- **Block C (close the loop, safely):** **26+** (extend the hallucination guard from coaching-only to the consolidation INPUT + ALL consumer outputs; enforce rep-private vs account-shared IN CODE, DEC-039) · **43** (refine loop) · **44** (the memory-loop keystone).
- **Block D (hard half of easy input):** **46** (AI intake router; subsumes the deferred Phase 36; propose-then-confirm, never a silent write to shared memory).
- **Then:** **40** (synced playback polish) — after the iOS device gate. Deferred post-launch: **22** (email).

**Why not the numeric order:** once the score feeds all of memory, a bad score or a misattributed fact poisons every downstream output. So three things MUST precede 44: (a) the scoring quality pass (garbage-in); (b) the guard extended beyond coaching (an ungrounded detail in a *summary* now reaches everything); (c) the rep-private/account-shared boundary enforced in code. And the voice ROUTER (46) writes to shared memory → it follows the mature guard + memory and must propose-then-confirm (Felix owns the routing/auto-commit policy, same class as 43/44).

**Remaining-to-prod (four buckets, 2026-06-23):** the feature build above is ONE bucket. (1) Feature build (this sequence; PR #39 38c/d + PR #40 scoring-retry MERGED to review-for-main 2026-06-23). (2) Beta-hardening — **28** consent/legal · **30** privacy+ToS/legal · **29** LogRocket+Sentry · **31** beta signup · **32** pre-launch sweep. (3) Prod infra — **separate prod Neon DB + Blob store** (DEC-029; prod shares ONE DB/store with dev+preview today) · Sentry DSN · Blob prod-scope. (4) **The promotion** — `review-for-main` → `master` is **frozen at Phase 13** (June 9, tip `3c2e198`); **45 commits / Phases 14–38 are verified on `review-for-main` but UNSHIPPED to prod** — mostly verification + the infra split, Felix-command-only. External long poles to start now: the **expert sales coach** (gates 44 + credibility) + **legal** (28/30).

## Phase 38 — Recording Analysis Page (keystone): playback + transcript + assessment ✅ DONE — core (PR #38) + 38c/38d (PR #39) merged 2026-06-23; single-segment shipped, multi-segment deferred (§D)

> **STATUS (2026-06-21 — PR [#38](https://github.com/FelixNava/Critiq2/pull/38) → review-for-main, squash `4ac5416`):** the non-audio core is MERGED — **38a** assign-flow inline-create fix (no navigation; pre-warm untouched) · **38b** `/recordings/[id]` owner-scoped page + honest pipeline states + status poller (backoff/cap/tab-blur) · **38e** full scorecard (overall + 3 pillar bars + 12 sub-dims + verbatim evidence + honest grounded/scored trust counter + inert overall-score refine) · **38f** scoring → **Opus 4.8** (caching confirmed LIVE on Opus, ~2.1k-tok methodology block, 72% hit — NO prompt growth needed; the advisory 4096 floor over-states like Sonnet's 2048 did; per-row model provenance NOT tracked = flip-the-constant) · **the value-path Blob-read 403 fix** (transcription read private chunks via unauthenticated `fetch` → 403 → 0 words → no score; now `@vercel/blob` `get(url,{access:'private'})`) · **readable + copyable transcript view** (Felix request; standalone, audio-independent; `TranscriptView`). Verified: typecheck+build clean; high-effort `/code-review` (8 angles) + `/design-critique` fixes applied in-branch; **Felix confirmed record → transcript → Opus assessment works END-TO-END live on the preview.** NO migration / no data-loss.
> **38c + 38d DONE + MERGED (single-segment) — PR [#39](https://github.com/FelixNava/Critiq2/pull/39) → review-for-main, squash `68e066e`, 2026-06-23 (Felix checked + authorized the merge).** `GET /api/recording/[id]/audio` server-proxies private chunk bytes with full HTTP Range (200/206/`Content-Range`/`Content-Length`, the iOS `bytes=0-1` probe, 416); a pure gap-aware `audioPlan` (dense single segment → `ready`; multi-segment / non-dense / empty → honest **422** + an explicit page message, never a corrupt concat); native `<audio controls>` player (the synced scrubber/markers/keyboard/speed = Phase 40); Content-Type inferred from the chunk pathname; real captured duration shown as a label (MediaRecorder-WebM has no in-header duration); reuses the Phase 38f authenticated private-read. **Verified on the preview against real recordings:** 401 unauth, 200 + valid WebM EBML, 206 iOS-probe + mid-range, two adjacent ranges concatenate byte-identical to the full body, 416 over-EOF, 422 honest-degrade, the `<audio>` element decodes (`readyState 4`). `scripts/verify-phase38c.ts` 41/41; build clean. **No schema / CSP / env change.** DEC-060.
> **38d shipped (single-segment):** synced transcript via `transcriptTimeline` (§C global-time re-basing, reuses `OVERLAP_MS`; click-to-seek + current-line highlight; speakers neutrally labeled per segment), `SyncedPlayback` shares one `<audio>` ref; `verify-phase38d.ts` 26/26. **STILL OPEN (deferred to the multi-segment build / Phase 40):** the **multi-segment** assembly boundary + the broader iOS path — lock/background playback, cross-boundary seek, MP4-origin decode at scale (`docs/CALL_ANALYSIS_AUDIO_CONTRACT.md` §D1–D4) — needs a real **≥10-min iPhone recording** (none of the existing recordings is multi-segment). §A–C are locked + shipped; §D is the empirical half.
...
## Phase 45 — Voice-input primitive (capture-is-effortless, the easy half) ☐ Planned (new 2026-06-23)
**Build:** a reusable voice-note affordance (mic → Deepgram transcribe → text) attachable to EVERY text input — debrief recap, account/rep context, import, pre-call narration. Reuses the existing `useRecorder` + Deepgram; adds a short single-shot voice-note transcribe path (the existing transcription is chunked-long-recording-oriented). The transcript drops into the SAME field the rep would have typed — no routing yet (that's 46). The first step toward "input should be easy."
**Depends on:** 15 (Deepgram). **Risk:** LOW–MEDIUM (a short-audio transcribe path + a shared input component). **Full-auto:** auto-build + interactive-verify; **the mic capture is device-gated (Felix).**

## Phase 46 — AI intake router (capture-is-effortless, the hard half) ☐ Planned (new 2026-06-23)
**Build:** "the AI figures out what goes where." A freeform voice/text blob is parsed + classified + ROUTED to the right sink(s): a debrief observation, an account-context fact, a commitment/next-step, an account-stage change, a rep note — and WHICH account it belongs to (subsumes the deferred Phase 36 auto-detect-account). **Propose-then-confirm:** the router surfaces what it would write where, the rep confirms — NEVER a silent write into shared, cross-fed memory (the Phase 26 guard + the rep-private/account-shared boundary apply to everything it would write).
**Depends on:** 26+ (extended guard), 44 (memory loop), 45 (voice). **Risk:** HIGH (misrouting writes wrong facts into shared memory that then feeds every consumer). **Full-auto:** plumbing auto behind a flag; **the routing + auto-commit-vs-confirm policy = human-only (Felix + expert coach)** — same calibration class as 43/44.
